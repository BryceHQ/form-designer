...
},wituiPlugins:[],plugins:["draggable","droppable","resizable","pagination","tooltip","linkbutton","menu","menubutton","splitbutton","progressbar","tree","textbox","filebox","combo","combobox","combotree","combogrid","numberbox","validatebox","searchbox","spinner","numberspinner","timespinner","datetimespinner","calendar","datebox","datetimebox","slider","layout","panel","datagrid","propertygrid","treegrid","tabs","accordion","window","dialog","form"],parse:function(_2){
var aa=[];
...
for(var i=0;i<$.parser.wituiPlugins.length;i++){
var _3=$.parser.wituiPlugins[i];
var r=$(".witui-"+_3,_2);
if(r.length){
if(r[_3]){
r[_3]();
}else{
aa.push({name:"witui-"+ _3,jq:r});
}
}
}
...
for(var ii=0;ii<opts.tools.length;ii++){
var tt=$("<a href=\"javascript:void(0)\"></a>").addClass(opts.tools[ii].iconCls).appendTo(tool);
if(opts.tools[ii].handler){
tt.bind("click",eval(opts.tools[ii].handler));
}
}
}else{
...
var tr=_281.find("tr");
for(var ii=0;ii<opts.toolbar.length;ii++){
var btn=opts.toolbar[ii];
if(btn=="-"){
...
_301(_30e);
_312._size("height",isNaN(opts.height)?(opts.height && opts.height.indexOf('%') > 0 ? (cc.height()-_311.outerHeight()) : "") :(opts.height-_311.outerHeight()));
...
var cpos={top:0,left:0,width:cc.width(),height:cc.height()};var marginTop = 0, marginBottom = 0, paddingTop = 0, paddingBottom = 0;try { marginTop = parseInt(cc[0].style.marginTop);marginTop=isNaN(marginTop)?0:marginTop; } catch (e) {marginTop=0; };try { marginBottom = parseInt(cc[0].style.marginBottom);marginBottom=isNaN(marginBottom)?0:marginBottom; } catch (e) { marginBottom=0;};try { paddingTop = parseInt(cc[0].style.paddingTop);paddingTop=isNaN(paddingTop)?0:paddingTop; } catch (e) {paddingTop=0;};try { paddingBottom = parseInt(cc[0].style.paddingBottom);paddingBottom=isNaN(paddingBottom)?0:paddingBottom; } catch (e) {paddingBottom=0; };cpos.height = cpos.height - marginTop - marginBottom - paddingTop - paddingBottom;
...
var east=_384(_382.expandEast)?_382.expandEast:(_382.east&&_382.east.length?_382.east:false),west=_384(_382.expandWest)?_382.expandWest:(_382.west&&_382.west.length?_382.west:false),north=_384(_382.expandNorth)?_382.expandNorth:(_382.north&&_382.north.length?_382.north:false),south=_384(_382.expandSouth)?_382.expandSouth:(_382.south&&_382.south.length?_382.south:false);var left=west?west.outerWidth()+_382.center.outerWidth():_382.center.outerWidth(),wholeWidth=(east?east.outerWidth():0)+left,containerWidth=cc.width();if(east){if(wholeWidth>containerWidth){east.panel("move", {left:  left});}};if(wholeWidth>containerWidth){resizeMargin(south,{width:wholeWidth});resizeMargin(north,{width:wholeWidth});};var topWithHead=north?north.parent().outerHeight()+_382.center.parent().outerHeight():_382.center.parent().outerHeight(),wholeHeight=_382.center.outerHeight(),wholeHeightWithHeader=(south?south.parent().outerHeight():0)+topWithHead,containerHeight=cc.outerHeight();if(south){if(wholeHeightWithHeader>containerHeight){south.panel("move", {top:  topWithHead});}};if(wholeHeightWithHeader>containerHeight){resizeMargin(west,{height:wholeHeight});resizeMargin(east,{height:wholeHeight});};function resizeMargin(panel,sizeOption){if(panel){panel.panel("resize", sizeOption);}};
...
pp.panel("move",{left:0,top:(type=="n"?0:cpos.top + cpos.height - _386)});
cpos.height-=_386;
if(type=="n"){
cpos.top+=_386;
...
$(_517).find(".calendar-title span")[0].innerHTML=opts.months[opts.month-1]+" "+opts.year;//$(_517).find(".calendar-title span").html(opts.months[opts.month-1]+" "+opts.year);
...
if(col.field){
td.attr("field",col.field);
if(col.sortable){td.append("<div class=\"datagrid-cell\"><span></span><span class=\"datagrid-sort-icon\"></span></div>");}else{td.append("<div class=\"datagrid-cell\"><span></span></div>");}
$("span",td)[0].innerHTML=col.title;//$("span",td).html(col.title);
if(col.sortable){$("span.datagrid-sort-icon",td).innerHTML="&nbsp;";}//if(col.sortable){$("span.datagrid-sort-icon",td).html("&nbsp;");}
...
var _678_value=$(_675).datagrid('getColumnOption',_678).valueField;
var ed=$.data(this,"datagrid.editor");
if(!_678_value){
ed.actions.setValue(ed.target,row[_678]);
}else{
ed.actions.setText(ed.target,row[_678]);
ed.actions.setValue(ed.target,row[_678_value]);
}
});
_679(_675,_676);
...
var _683_value=$(_67b).datagrid('getColumnOption',_683).valueField;
var ed=$.data(this,"datagrid.editor");
var t=$(ed.target);
if(!_683_value){
...
_682[_683]=_685;
	...
	var _685_value=ed.actions.getValue(ed.target);
	var _685_text=ed.actions.getText(ed.target);
	if(row[_683]!=_685_text || row[_683_value]!=_685_value){
		row[_683]=_685_text;
		row[_683_value]=_685_value;
		_681=true;
		_682[_683]=_685_text;
		_682[_683_value]=_685_value;
	}
}
});
if(_681){
...
$(_72a)[0].innerHTML = _72e.join("");//$(_72a).html(_72e.join(""));
...
$(_734)[0].innerHTML=_737.join("");//$(_734).html(_737.join(""));
...
tr.find("div.datagrid-cell-rownumber")[0].innerHTML=_75a;//tr.find("div.datagrid-cell-rownumber").html(_75a);
}
if(opts.striped){
...
tr.find("div.datagrid-cell-rownumber")[0].innerHTML=_766;//tr.find("div.datagrid-cell-rownumber").html(_766);
}
if(opts.striped){
tr.removeClass("datagrid-row-alt").addClass((i-1)%2?"datagrid-row-alt":"");
}
...
$(_7b7)[0].innerHTML = _7b9.join("");//$(_7b7).html(_7b9.join(""));
...
span[0].innerHTML=opts.groupFormatter.call(_7dc,_7df.value,_7df.rows);//span.html(opts.groupFormatter.call(_7dc,_7df.value,_7df.rows));
...
$(this)[0].innerHTML=i+1;//$(this).html(i+1);
});
};
function _81c(_81d){
return function(e){
...
$(_89e)[0].innerHTML=_8a1.join("");//$(_89e).html(_8a1.join(""));
...
tr[0].innerHTML=this.renderRow(_8ab,_8b3,_8b2,_8ad,_8ac);//tr.html(this.renderRow(_8ab,_8b3,_8b2,_8ad,_8ac));
tr.attr("style",_8ae||"");
tr.find("div.datagrid-cell-rownumber")[0].innerHTML=_8b4;//tr.find("div.datagrid-cell-rownumber").html(_8b4);
if(_8b5){
tr.find("div.datagrid-cell-check input[type=checkbox]")._propAttr("checked",true);
}
if(_8b0!=id){
...
var _8dc=$.extend(true,[],opts.icons);if (!opts.editable && !opts.required) {_8dc.push({ iconCls: "combo-clear", handler: function (e) {_8f6(e.data.target);}});}
...
_913=_913||{};
return this.each(function(){
var ii=$.data(this,"combo");
if(ii){
$.extend(ii.options,_913);
if(_913.value!=undefined){
ii.options.originalValue=_913.value;
}
}else{
ii=$.data(this,"combo",{options:$.extend({},$.fn.combo.defaults,$.fn.combo.parseOptions(this),_913),previousText:""});
ii.options.originalValue=ii.options.value;
}
_8d5(this);
_910(this);
});
...
$(_938).combo("panel")[0].innerHTML=dd.join("");//$(_938).combo("panel").html(dd.join(""));
if(opts.multiple){
_92d(_938,_93b,_939);
}else{
